...
from obtain import Spider
from db import RedisClient
from multiprocessing import Process


class Scheduler:
    __Test_Url = ['https://www.baidu.com', 'https://www.so.com', 'http://www.soso.com', 'https://www.sogou.com']
    ...
    def get_proxy_ip(self):
        ...
        while True:
            ...
            if db.count() > 200:
                print('代理数量已达标 %s, 等待10分钟' % db.count())
                crawl.sleep(10*60)
            # 当前时间
    ...
    async def save_proxy_ip(self, semaphore, ip, type = 'save'):
        ...
        async with semaphore:
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    #开始验证代理
                    async with session.get(random.choice(self.__Test_Url), proxy=proxy, headers=Spider().crate_headers(), timeout=3) as response:
                        #如果代理成功
